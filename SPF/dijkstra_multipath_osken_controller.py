...
from dijkstra_osken_controller import SPF_FLOW_COOKIE
from dijkstra_osken_controller import SPF_FLOW_COOKIE_MASK
from dijkstra_osken_controller import SPF_FLOW_PRIORITY
from dijkstra_osken_controller import DijkstraSwitch
...
class DijkstraMultipathSwitch(DijkstraSwitch):
    """Dijkstra SPF with equal-cost multipath forwarding."""
    ...
    def _dijkstra_multi_parent(self, src, dst):
        """Run Dijkstra once and keep all equal-cost predecessors."""
        ...
        self.logger.debug("[SPF-MP-INIT] src=s%d dst=s%d vertices=%d edges=%d",
        ...
        self.logger.info("[SPF-MP-DONE] s%d->s%d edge_exams=%d relaxations=%d shortest_cost=%s",
                         src, dst, edge_exams, relaxations,
    ...
    def compute_multipath(self, src, dst, first_port, final_port, k_paths=K_PATHS):
        ...
        if cache_key in self.path_cache:
            ...
            distance, parents = self._dijkstra_multi_parent(src, dst)
    ...
    def install_multipath(self, paths, src_mac, dst_mac):
        ...
        # Install deterministic forwarding on non-ingress hops first.
        for path in paths:
